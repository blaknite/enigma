...
module Enigma
  class PlugBoard
    ...
    def build_plug_pair(plug_pair, plug_pairs)
      raise StandardError, "plug already connected to #{plug_pair[0]}" if plug_pairs.key?(plug_pair[0])
      raise StandardError, "plug already connected to #{plug_pair[1]}" if plug_pairs.key?(plug_pair[1])
